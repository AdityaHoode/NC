class Solution:
    def findMedianSortedArrays(self, nums1: List[int], nums2: List[int]) -> float:
        ...
        while left<=right:
            ...
            a_right = float('-inf') if mid_a==0 else nums_a[mid_a-1]
            b_right = float('-inf') if mid_b==0 else nums_b[mid_b-1]
            a_left = float('inf') if mid_a==len_a else nums_a[mid_a]
            b_left = float('inf') if mid_b==len_b else nums_b[mid_b]
